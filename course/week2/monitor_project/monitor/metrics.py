...
def get_vocab_outlier(tr_vocab, te_vocab):
  ...
  # score: float (between 0 and 1)
  num_seen = len([w for w in te_vocab if tr_vocab.get(w,0)>0])
...
class MonitoringSystem:
  ...
  def calibrate(self, tr_probs, tr_labels, te_probs):
    ...
    r = IsotonicRegression(out_of_bounds="clip")
    tr_probs_cal = torch.from_numpy(r.fit_transform(tr_probs, tr_labels))
    te_probs_cal = torch.from_numpy(r.transform(te_probs))
    # ============================
